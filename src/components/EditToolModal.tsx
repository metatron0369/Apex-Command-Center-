...
export default function EditToolModal({
  ...
  useEffect(() => {
    if (isOpen && tool) {
      ...
      setCustomLogoUrl(tool.customLogoUrl || '');
      setDescription(tool.description || '');
      setIsPinned(!!tool.isPinned);
      setIsLocalApp(!!tool.isLocalApp);
      setLocalPath(tool.localPath || '');
    }
  }, [isOpen, tool]);
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/95 shadow-2xl p-6 text-slate-100 flex flex-col gap-6 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold font-display tracking-tight text-white uppercase animate-pulse">Edit Command Portal</h2>
              <p className="subtitle text-xs text-slate-400">Modify properties or transfer categories of this card.</p>
            </div>
          </div>
          <button 
            type="button" 
        ...
        {isUrlValid && (
          <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-950/50 border border-slate-800">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-xs font-mono text-gold-400 uppercase tracking-widest block font-medium">Automatic Logo Mapping</span>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-h-[60vh] overflow-y-auto pr-1">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-slate-300 font-mono text-gold-400">Portal Target Protocol *</label>
          ...
          {!isLocalApp ? (
            ...
            <div className="flex flex-col gap-1.5 animate-fade-in">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500 font-mono text-gold-500">e.g. slack://, obsidian://, or raw path</span>
              </div>
              <input
                type="text"
                required={isLocalApp}
